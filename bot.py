...
async def yt_webhook_handler(request: web.Request):
    ...
    # Announce only if not already seen
    if vid and title:
...
async def announce_youtube(video_id: str, title: str):
    """Announce a new YT video with your exact phrasing & Upload Ping role, but skip if already announced."""
    ...
    # Skip duplicate video IDs (prevents repost after restart & duplicate webhooks)
    cache = load_yt_cache()
    last_vid = cache.get("last_video_id")
    if last_vid and last_vid == video_id:
        return
...
async def announce_x(tweet_url: str, title_text: str | None):
    ...
    # Skip duplicate tweet IDs
    cache = load_x_cache()
    last_tweet_id = cache.get("last_tweet_id")
    m = re.search(r"/status/(\d+)", tweet_url)
    this_id = m.group(1) if m else None
    if last_tweet_id and this_id and this_id == last_tweet_id:
        return
    ...
    try:
        await ch.send(content=content, embed=embed, allowed_mentions=allowed)
        if this_id:
            save_x_cache({"last_tweet_id": this_id})
    except Exception:
        pass
...
# ================== STARTUP CACHE PRIMERS (prevent repost on restart) ==================
async def init_latest_youtube_cache():
    """On cold start, fetch latest YT item and set it as last_video_id without announcing."""
    feed_url = f"https://www.youtube.com/feeds/videos.xml?channel_id={YT_CHANNEL_ID}"
    try:
        async with aiohttp.ClientSession(headers={"User-Agent":"Mozilla/5.0"}) as session:
            async with session.get(feed_url, timeout=10) as resp:
        ...
        if vid:
            cache = load_yt_cache()
            if cache.get("last_video_id") != vid:
                save_yt_cache({"last_video_id": vid})
    except Exception as e:
        print(f"[init_yt_cache] error: {e}")

async def init_latest_x_cache():
    """On cold start, fetch latest tweet id and set last_tweet_id without announcing."""
    # Try RSS first
    feeds = [X_RSS_URL] + [u for u in X_RSS_FALLBACKS if u != X_RSS_URL]
    try:
        xml = None
        ...
        if xml:
            try:
                ...
                if items:
                    item = items[0]
                    link  = (item.findtext("link")  or "").strip()
                    tweet_id = _extract_tweet_id(link) or nitter_latest_id_from_html(link)
                    if tweet_id:
                        cache = load_x_cache()
                        if cache.get("last_tweet_id") != tweet_id:
                            save_x_cache({"last_tweet_id": tweet_id})
                        return
            except Exception:
                pass
...
async def on_ready():
    ...
    # Prime caches to avoid reposting most-recent YT/X on restart
    await init_latest_youtube_cache()
    await init_latest_x_cache()
...
async def on_message(message: discord.Message):
    ...
    # ---------- NEW FEATURE: Newcomer link block ----------
    if in_guild:
        newcomer_role = message.guild.get_role(NEWCOMER_ROLE_ID)
        if newcomer_role and newcomer_role in message.author.roles:
            # Block URLs for Newcomers (http/https, www., discord invites)
            if re.search(r"(https?://|www\\.|discord\\.gg/|discord\\.com/invite/)", content, re.I):
                try:
                    await message.delete()
                except Exception:
                    pass
                try:
                    ...
                    pass
                return  # stop further handling ONLY when a link was posted
